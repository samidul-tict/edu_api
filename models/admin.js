...
adminSchema.methods.generateAuthToken = function (role) {
    ...
    }, config.get('jwtPrivateKey'), { expiresIn: '6h' });
    return token;
}
...
function validateAdmin(admin) {
    const schema = {
        ...
        password: Joi.string()
            .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,12}$/) // you have to put it in this way and it will work :)
            .required()
            .error(() => {
                return {
                    message: 'Password Should be between 6-12 characters and consist uppercase, lowercase, number and special characters',
                };
            }),
